...
const CrntMoviePage = () => {
  ...
  const crntList = useSelector((state: RootState) => state.thumbList.thumbList);
  console.log(crntList);

  const handleUrl = (movieName: any) => {
    navigate(`curMovie/${movieName}`)
  }
  ...
  return <CrntBox>
    <ul>
      {crntList.map((v, i) =>
        <li onClick={() => handleUrl(v.movieNm)}>
          <div className="rank">
            {i + 1}
          </div>
